...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Si no estamos en la pantalla de inventario, no hacemos nada
    if (!listbox && !btnGuardar) return;
    ...
    if (listbox) {
        listbox.addEventListener("change", async () => {
            ...
            console.log('ID COMPRA SELECCIONADO:', currentCompraId);
            if (!currentCompraId) return;

            contenedorLotes.innerHTML = "<p>Cargando detalle...</p>";

            try {
                //const res = await fetch(`/inventario/compra/${currentCompraId}/detalle`); mazucena
                const res = await fetch(
                `${window.APP_URL}/inventario/compra/${currentCompraId}/detalle`
);

                const data = await res.json();

                if (!data.success) {
                    alert(data.message || "Error al cargar la compra");
                    return;
                }
...
async function generarTarjeta(prod) {
            ...
            <div class="form-lote" data-id="${prod.idProductos}">
                ...
                <div class="grid-lote">
                    <div>
                        ...
                        <input type="text" value="${codigoLote}">
                    </div>
    ...
    // ============================
    // 3. AGREGAR / ELIMINAR PIEZAS
    // ============================
    document.addEventListener("click", (e) => {

        if (e.target.classList.contains("btn-agregar-pieza")) {
            agregarPieza(e.target.dataset.id);
        }
...
if (btnGuardar) {
    btnGuardar.addEventListener("click", async () => {
        try {
            ...
            const res = await fetch(`${window.APP_URL}/inventario/automatico/guardar`, {
                method: "POST",
                headers: {
