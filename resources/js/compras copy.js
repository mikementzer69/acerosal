...
radiosMoneda.forEach(radio => {
    ...
    function crearBloqueFamilia(idFam, nombre) {
        ...
        // AQUÍ MODIFICAMOS LOS ENCABEZADOS DE LA TABLA
bloque.innerHTML = `
...
function agregarFilaProducto(bloque, idFam) {
    ...
    const inputCant      = tr.querySelector('.input-cantidad'); // 👈 Capturamos la cantidad
    const inputsCalc     = tr.querySelectorAll('.campo-prod');
    const btnEliminar    = tr.querySelector('.btn-eliminar-fila');

    // --- ✨ NUEVO: DETECTOR DE EDICIÓN MANUAL ✨ ---
    inputCant.addEventListener('input', (e) => {
        // Si el evento tiene 'inputType', significa que el usuario escribió o borró algo
        if (e.inputType) {
            inputCant.classList.remove('is-auto'); // Quitamos la marca automática
            inputCant.style.color = "#000";       // Negro normal para manual
        }
    });
    ...
    // ✨ Ahora creamos las 8 columnas exactas de tu resumen ✨
    tr.innerHTML = `
        <td>${nombre}</td>
    ...
    // ==========================================================
    // RECÁLCULO GENERAL (MODIFICADO PARA IGNORAR EUROS)
    // ==========================================================
...
function gestionarColumnasVisibles(moneda) {
    ...
    // Aplicar estilo visual de "bloqueado"
...
// ✨ Llamalo cada vez que cambien la moneda y al agregar una fila nueva
...
function recalcularTotales() {
    ...
    // 2. Recorrer bloques de familias y sus productos
    contenedorFamilias.querySelectorAll('.bloque-familia').forEach(b => {
        ...
        b.querySelectorAll('tbody tr').forEach(tr => {
            ...
            // 🕵️ ¿El valor actual lo puso el sistema?
            ...
            // 🔮 LÓGICA DE CÁLCULO AUTOMÁTICO REFINADA
            // Si el peso es 0 y el campo era automático, lo limpiamos
            if (pesoKG === 0 && esAuto) {
                inputCant.value = "";
                inputCant.classList.remove('is-auto');
            }
            // Si el campo está en 0 O es automático, calculamos
                ...
                inputCant.classList.add('is-auto'); // El sistema mantiene el control
                inputCant.style.color = "#60a5fa";  // Color azul "calculado"
            }
            else if (cantMetros > 0 && !esAuto) {
                inputCant.style.color = "#000"; // Negro si es manual
            }

            // --- Cálculos de Pesos e Importes ---
    ...
    // 5. Actualizar Resumen por Familia
