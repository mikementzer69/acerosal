...
<div class="erp-section">
    ...
    <h2 class="erp-title">
        <i class="fa-solid fa-book"></i> Editar Cuenta Contable
    </h2>
        ...
        <form method="POST" action="{{ route('cuentas.update', $cuenta->id) }}">
            ...
            <table class="erp-table" style="border:none;">
                ...
                <tr>
                    ...
                    <td>
                        <select name="tipo" class="search-input" required>
                            @foreach(['activo','pasivo','patrimonio','ingreso','gasto'] as $t)
                    ...
                    <td>
                        <select name="parent_id" class="search-input">
                            ...
                            @foreach($cuentas_padre as $p)
                                <option value="{{ $p->id }}"
                                    {{ $cuenta->parent_id == $p->id ? 'selected' : '' }}>
                                    {{ $p->codigo }} — {{ $p->nombre }}
                                </option>
                            @endforeach
                        </select>
                ...
                <tr>
                    ...
                    <td>
                        <input type="checkbox" name="es_movimiento"
                               {{ $cuenta->es_movimiento ? 'checked' : '' }}>
