...
<div class="form-container">
    ...
    <h2 class="form-title">
        <i class="fa-solid fa-user-tie"></i> Nuevo Cliente
    </h2>
    ...
    <form method="POST"
        ...
        @csrf
        {{-- No necesitamos @method('PUT') porque es un POST normal --}}
        ...
        <div style="display: flex; gap: 30px; margin-bottom: 20px; flex-wrap: wrap;">
            ...
            {{-- Código (Solo lectura - Gris oscuro para diferenciar) --}}
            ...
            <div style="flex: 1; min-width: 150px;">
                ...
                <select name="tipo_cliente" style="width: 100%; height: 45px; padding: 5px 10px; background-color: #111; color: #fff; border: 1px solid #444; border-radius: 4px;">
                    ...
                    <option value="JURIDICO" {{ old('tipo_cliente') == 'JURIDICO' ? 'selected' : '' }}>Jurídico (Empresa)</option>
                    <option value="EXENTO" {{ old('tipo_cliente') == 'EXENTO' ? 'selected' : '' }}>Contribuyente Exento</option>
                </select>
            </div>
            ...
            <div style="flex: 1; min-width: 120px;">
                ...
                <select name="origen" style="width: 100%; height: 45px; padding: 5px 10px; background-color: #111; color: #fff; border: 1px solid #444; border-radius: 4px;">
                    <option value="N" {{ old('origen') == 'N' ? 'selected' : '' }}>Nacional</option>
                    <option value="E" {{ old('origen') == 'E' ? 'selected' : '' }}>Extranjero</option>
                </select>
            </div>
            ...
            <div style="flex: 1; min-width: 150px;">
                ...
                <select name="tipo_contribuyente" style="width: 100%; height: 45px; padding: 5px 10px; background-color: #111; color: #fff; border: 1px solid #444; border-radius: 4px;">
                    <option value="O" {{ old('tipo_contribuyente') == 'O' ? 'selected' : '' }}>Otros (O)</option>
                    <option value="P" {{ old('tipo_contribuyente') == 'P' ? 'selected' : '' }}>Pequeño (P)</option>
                    <option value="M" {{ old('tipo_contribuyente') == 'M' ? 'selected' : '' }}>Mediano (M)</option>
                    <option value="G" {{ old('tipo_contribuyente') == 'G' ? 'selected' : '' }}>Grande (G)</option>
                </select>
            </div>
            ...
            <div style="flex: 1; min-width: 180px;">
                ...
                <input type="text" name="documento" value="{{ old('documento') }}" placeholder="00000000-0"
            ...
            <div style="flex: 1; min-width: 180px;">
                ...
                <input type="text" name="nit" value="{{ old('nit') }}" placeholder="0000-000000-000-0"
                       style="width: 100%; padding: 10px; background-color: #111; color: #fff; border: 1px solid #444; border-radius: 4px;">
            </div>
            ...
            <div style="flex: 1; min-width: 150px;">
                ...
                <input type="text" name="nrc" value="{{ old('nrc') }}" placeholder="123456-7"
                       style="width: 100%; padding: 10px; background-color: #111; color: #fff; border: 1px solid #444; border-radius: 4px;">
            </div>
            ...
            <div style="flex: 1; min-width: 250px;">
                ...
                <select name="id_giro" style="width: 100%; height: 45px; padding: 5px 10px; background-color: #111; color: #fff; border: 1px solid #444; border-radius: 4px;">
                    ...
                    @foreach($giros as $giro)
                        <option value="{{ $giro->id }}" {{ old('id_giro') == $giro->id ? 'selected' : '' }}>
        ...
        <div style="display: flex; gap: 30px; margin-bottom: 20px; flex-wrap: wrap;">
            <div style="flex: 1; min-width: 200px;">
                ...
                <input type="text" name="nite" value="{{ old('nite') }}" placeholder="Opcional..."
                       style="width: 100%; padding: 10px; background-color: #111; color: #fff; border: 1px solid #444; border-radius: 4px;">
            </div>
            <div style="flex: 1; min-width: 200px;">
                <label style="display:block; margin-bottom: 8px; color: #ddd;">Pasaporte</label>
                <input type="text" name="pasaporte" value="{{ old('pasaporte') }}" placeholder="Opcional..."
                       style="width: 100%; padding: 10px; background-color: #111; color: #fff; border: 1px solid #444; border-radius: 4px;">
            </div>
        </div>
        ...
        <div style="display: flex; gap: 30px; margin-bottom: 20px; flex-wrap: wrap;">
            ...
            <div style="flex: 1; min-width: 200px;">
                ...
                <input type="text" name="contacto_principal" value="{{ old('contacto_principal') }}" placeholder="Encargado de compras"
                       style="width: 100%; padding: 10px; background-color: #111; color: #fff; border: 1px solid #444; border-radius: 4px;">
            </div>
        </div>

        {{-- LÍNEA 5: DEPARTAMENTO | CIUDAD --}}
        <div style="display: flex; gap: 30px; margin-bottom: 20px; flex-wrap: wrap;">
            <div style="flex: 1; min-width: 200px;">
            ...
            <div style="flex: 1; min-width: 200px;">
                <label style="display:block; margin-bottom: 8px; color: #ddd;">Ciudad / Municipio</label>
        ...
        {{-- LÍNEA 6: LÍMITE | DÍAS | ESTADO --}}
        <div style="display: flex; gap: 30px; margin-bottom: 20px; flex-wrap: wrap;">
            <div style="flex: 1; min-width: 150px;">
                <label style="display:block; margin-bottom: 8px; color: #ddd;">Límite Crédito ($)</label>
                <input type="number" step="0.01" name="limite_credito" value="{{ old('limite_credito', '0.00') }}"
                       style="width: 100%; padding: 10px; background-color: #111; color: #fff; border: 1px solid #444; border-radius: 4px;">
            </div>
            <div style="flex: 1; min-width: 150px;">
                <label style="display:block; margin-bottom: 8px; color: #ddd;">Días Crédito</label>
                <input type="number" name="dias_credito" value="{{ old('dias_credito', '0') }}"
                       style="width: 100%; padding: 10px; background-color: #111; color: #fff; border: 1px solid #444; border-radius: 4px;">
            </div>
            <div style="flex: 1; min-width: 150px;">
            ...
            <div style="flex: 1; min-width: 150px; display: flex; align-items: center; margin-top: 25px;">
                <input type="checkbox" name="exento" value="1" {{ old('exento') ? 'checked' : '' }} id="exento" style="width: 20px; height: 20px; margin-right: 10px; cursor: pointer;">
