...
class CompraController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // ✨ MAGO: Si eligieron múltiple y no llenaron el campo factura general, le ponemos un comodín.
        $numeroFacturaGeneral = $request->numero_factura;
        if ($request->tipo_facturacion == 'multiple') {
            $numeroFacturaGeneral = 'FACTURAS MÚLTIPLES';
        } elseif (empty($numeroFacturaGeneral)) {
             return back()->with('error', 'El número de factura general es obligatorio.')->withInput();
        }
        ...
        try {
            ...
            $compra = Compra::create([
                ...
                'numero_factura'         => $numeroFacturaGeneral,
            ...
            foreach ($productosPorFamilia as $idF => $v) {
                ...
                $facturaFamilia = $numeroFacturaGeneral;
                if ($request->tipo_facturacion == 'multiple' && isset($request->factura_familia[$idF])) {
                    $facturaFamilia = $request->factura_familia[$idF];
                }

                CompraFamilia::create([
                    'id_compra'              => $compra->id_compra,
                    'id_familia'             => $idF,
                    'numero_factura'         => $facturaFamilia, // ✨ MAGO: NUEVO CAMPO. ¡Asegurate de agregarlo a la migración/base de datos!
